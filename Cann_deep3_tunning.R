...
model_deep3_cann<- keras_model(inputs = c(Design, Brand, Region, LogVol), outputs = c(Response))



model_deep3_cann %>% compile(optimizer = "nadam", loss = Poisson.Deviance)


hist_deep3_cann <- model_deep3_cann %>% 
...
data_fit <- as.data.frame(hist_deep3_cann)


ggplot(data_fit[which(!is.na(data_fit$value)),],aes(x=epoch,y=value,col=data))+
  geom_point()

score_cann_deep3 <- model_deep3_cann %>% 
