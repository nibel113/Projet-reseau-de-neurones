...
##  lambda.hom = 1
##  Maintenant le offset est la prévision du glm, qui tient compte de l'exposition
Vlearn <- as.matrix(log(learnglm$fit))
Vtest <- as.matrix(log(testglm$fit))
Vval <- as.matrix(log(valglm$fit))
lambda.hom <- sum(learnNN$ClaimNb)/sum(learnglm$fit)
...
Network = list(Design, BrandEmb, RegionEmb) %>% 
  ...
  layer_dense(units = 1, 
              ...
              weights = list(array(0, dim=c(FLAGS$hidden4,1)), 
                             # l'initialisation fait que les poids sont à 0
                             # et le biais est à log(1)=0
                             # donc cette partie du réseau à la première phase de propagation est égale à 0
                             array(log(lambda.hom), dim=c(1))
              )
  )

## On ajoute le offset défini plus haut
## ainsi l'initialisation du réseau au total est égale 
## au MVE du glm
...
model_deep4_cann<- keras_model(inputs = c(Design, Brand, Region, LogVol), outputs = c(Response))



model_deep4_cann %>% compile(optimizer = "nadam", loss = Poisson.Deviance)


hist_deep4_cann <- model_deep4_cann %>% 
...
data_fit <- as.data.frame(hist_deep4_cann)


ggplot(data_fit[which(!is.na(data_fit$value)),],aes(x=epoch,y=value,col=data))+
  geom_point()

score_cann_deep4 <- model_deep4_cann %>% 
