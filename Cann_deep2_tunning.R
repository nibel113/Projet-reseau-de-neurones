...
model_deep2_cann<- keras_model(inputs = c(Design, Brand, Region, LogVol), outputs = c(Response))



model_deep2_cann %>% compile(optimizer = "nadam", loss = Poisson.Deviance)


hist_deep2_cann <- model_deep2_cann %>% 
...
data_fit <- as.data.frame(hist_deep2_cann)


ggplot(data_fit[which(!is.na(data_fit$value)),],aes(x=epoch,y=value,col=data))+
  geom_point()

score_cann_deep2 <- model_deep2_cann %>% 
