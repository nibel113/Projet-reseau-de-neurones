...
{t1 <- proc.time()
  fit <- model %>% fit(list(Xlearn_all, Wlearn_all), Ylearn_all, epochs=74, batch_size=6611,validation_data=list(list(XtestNN,WtestNN),YtestNN))
  (proc.time()-t1)}
data_fit <- as.data.frame(fit)
plot(data_fit$value[165:320])
which.min(data_fit$value[161:320])
...
net <- features.0 %>%
  #on ajoute kernel initialize direct dans le dense layer
  layer_dense(units = q1, activation = 'relu',kernel_initializer = initializer_he_normal(),kernel_regularizer = regularizer_l1(l=0.001)) %>% 
  layer_dense(units = 1, activation = k_exp,kernel_initializer = initializer_he_normal())
volumes.0 <- layer_input(shape=c(1))                     # define network for offset
offset <- volumes.0 %>%
  layer_dense(units = 1, activation = 'linear', use_bias=FALSE, trainable=FALSE, weights=list(array(1, dim=c(1,1))))
merged <- list(net, offset) %>%                          # combine the two networks
  layer_multiply() 
model <- keras_model(inputs=list(features.0, volumes.0), outputs=merged)    
save_model_weights_hdf5(model,"model_reg.hdf5")
summary(model)    
...
which.min(data_fit$value[401:800])
plot(data_fit$value[420:800])
plot(data_fit$value[20:400])
ggplot(data_fit,aes(x=epoch,y=value,col=data))+
  geom_point()+
  scale_y_continuous(limits = c(0.16,.170))+
...
{t1 <- proc.time()
  fit <- model %>% fit(list(Xlearn_all, Wlearn_all), Ylearn_all, epochs=350, batch_size=6611,validation_data=list(list(XtestNN,WtestNN),YtestNN))
  (proc.time()-t1)}

learnNN$fit.keras <- as.vector(model %>% predict(list(Xlearn_all, Wlearn_all)))
testNN$fit.keras <- as.vector(model %>% predict(list(XtestNN, WtestNN)))
names <- c(names,"SN_20_reg_l1_0.001")
out_loss <- c(out_loss,Poisson.Deviance(testNN$fit.keras,testNN$ClaimNb))
in_loss <- c(in_loss,Poisson.Deviance(learnNN$fit.keras,learnNN$ClaimNb))
...
{t1 <- proc.time()
  fit <- model %>% fit(list(Xlearn_all, Wlearn_all), Ylearn_all, epochs=244, batch_size=6611,validation_data=list(list(XtestNN,WtestNN),YtestNN))
  (proc.time()-t1)}

data_fit <- as.data.frame(fit)

which.min(data_fit$value[245:448])
...
{t1 <- proc.time()
  fit <- model %>% fit(list(Xlearn_all, Wlearn_all), Ylearn_all, epochs=160, batch_size=6611,validation_data=list(list(XtestNN,WtestNN),YtestNN))
  (proc.time()-t1)}

learnNN$fit.keras <- as.vector(model %>% predict(list(Xlearn_all, Wlearn_all)))
testNN$fit.keras <- as.vector(model %>% predict(list(XtestNN, WtestNN)))
names <- c(names,"SN_20_drop_005")
out_loss <- c(out_loss,Poisson.Deviance(testNN$fit.keras,testNN$ClaimNb))
in_loss <- c(in_loss,Poisson.Deviance(learnNN$fit.keras,learnNN$ClaimNb))
...
{t1 <- proc.time()
  fit <- model %>% fit(list(Xlearn_all, Wlearn_all), Ylearn_all, epochs=166, batch_size=6611,validation_data=list(list(XtestNN,WtestNN),YtestNN))
  (proc.time()-t1)}

learnNN$fit.keras <- as.vector(model %>% predict(list(Xlearn_all, Wlearn_all)))
testNN$fit.keras <- as.vector(model %>% predict(list(XtestNN, WtestNN)))
names <- c(names,"SN_20_drop_001")
out_loss <- c(out_loss,Poisson.Deviance(testNN$fit.keras,testNN$ClaimNb))
in_loss <- c(in_loss,Poisson.Deviance(learnNN$fit.keras,learnNN$ClaimNb))
...
save_model_weights_hdf5(model,"model_S10.hdf5")
...
load_model_weights_hdf5(model,"model_S10.hdf5")
{t1 <- proc.time()
  fit <- model %>% fit(list(Xlearn_all, Wlearn_all), Ylearn_all, epochs=155, batch_size=6611,validation_data=list(list(XtestNN,WtestNN),YtestNN))
  (proc.time()-t1)}
...
save_model_weights_hdf5(model,"model_S30.hdf5")
...
load_model_weights_hdf5(model,"model_S30.hdf5")
{t1 <- proc.time()
  fit <- model %>% fit(list(Xlearn_all, Wlearn_all), Ylearn_all, epochs=100, batch_size=6611,validation_data=list(list(XtestNN,WtestNN),YtestNN))
  (proc.time()-t1)}

learnNN$fit.keras <- as.vector(model %>% predict(list(Xlearn_all, Wlearn_all)))
testNN$fit.keras <- as.vector(model %>% predict(list(XtestNN, WtestNN)))
names <- c(names,"SN_30_epo100")
out_loss <- c(out_loss,Poisson.Deviance(testNN$fit.keras,testNN$ClaimNb))
in_loss <- c(in_loss,Poisson.Deviance(learnNN$fit.keras,learnNN$ClaimNb))
...
save_model_weights_hdf5(model,"model_S40.hdf5")
...
load_model_weights_hdf5(model,"model_S40.hdf5")
{t1 <- proc.time()
  fit <- model %>% fit(list(Xlearn_all, Wlearn_all), Ylearn_all, epochs=70, batch_size=6611,validation_data=list(list(XtestNN,WtestNN),YtestNN))
  (proc.time()-t1)}
