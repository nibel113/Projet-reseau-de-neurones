...
model_deep3_embed <- keras_model(inputs = c(Design, Brand, Region, LogVol), outputs = c(Response))



model_deep3_embed %>% compile(optimizer = "nadam", loss = Poisson.Deviance)


hist_deep3_embeded <- model_deep3_embed %>% 
...
data_fit <- as.data.frame(hist_deep3_embeded)


ggplot(data_fit[which(!is.na(data_fit$value)),],aes(x=epoch,y=value,col=data))+
  geom_point()

score_deep3_embed <- model_deep3_embed %>% 
